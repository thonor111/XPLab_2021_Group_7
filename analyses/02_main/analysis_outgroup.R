...
#Read in data
full_data <- read_csv("clean_data.csv")
...
useable_data <- useable_data %>%
  ...
  mutate(ingroupAgree = ifelse(is.na(ingroupAgree), 1, ingroupAgree), #For experiment 1 we assumed that participants identified with the ingroup and not with the outgroup.
...
stan_data2 <- as.list(c(filter(useable_data, expNum=="Exp 1"), N=sum(useable_data$expNum=="Exp 1")))
...
issue_plot_1 <- ggplot(data1, aes(x=factor(topIssue, labels = c("Gun \n control", 
                                                                "Feminism", 
                                                                "Joe \n Biden", 
                                                                "Immigration",
                                                                "Transgender \n rights",
...
ggsave("issue_plot_1.png", issue_plot_1, width=34, height=13, units="cm")
